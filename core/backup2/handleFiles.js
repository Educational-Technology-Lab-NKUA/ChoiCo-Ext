...
newGame.prototype.updatePoint = function (pointID, newValue, fieldID, origin, newMark,cel) {
  ...
  if(point!= undefined){
      ...
      if (fieldID == 0)
       ...
       { if (origin == "system")    //called by newEntry function
...
newGame.prototype.getMapSettings = function () {
  if(usingGoogleMaps){
     var mapSets = {zoom: this.myMap.mapInstance.getZoom(),bounds: findSaveBounds(), googleMaps : true, googleMarkersCounter:googleMarkersCounter}
...
newGame.prototype.checkIntialForBugs = function () {
  ...
  for (i =0; i < w1Blocks.length; i ++){
    curBlock = w1Blocks[i];
  	if (curBlock.type== "initialValue" ){
       ...
       addConsoleMessage ("Error: You have to set a number for the attribute "+fieldName+ " at the Inital Settings blocks (tab2)");
       bugsFound = true;
     }
  ...
  for (i=0; i < this.variables.length; i++){
    ...
    if (!found){
    addConsoleMessage ("Error: You have not set the initial value of attribute "+this.variables[i].name + " at the Inital Settings tab (tab1)");
       bugsFound = true;
    }
  }
  for (i=0; i < blocklyVars.length; i++){
    ...
    if(this.checkBlocklyVar (blocklyVars[i].name))
      found = true;
    if (!found){
    addConsoleMessage (language.variableErrorMessageA + blocklyVars[i].name + language.variableErrorMessageB)
       bugsFound = true;
    }
  }
}
...
//	console.log (f.type);
  ...
  addConsoleMessage = function (message) {
    ...
    var newcontent = "<p>" + message + "</p>" + oldcontent;
