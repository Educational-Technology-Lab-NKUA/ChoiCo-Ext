...
newGame.prototype.resetOnPlatform = function (gameState) {
	this.idCounter = 0;
	platformReload = true;
	mode = 2;
	...
	//handle the maps
	if (gameState.mapSettings.googleMaps && usingGoogleMaps){	//already using maps version
	...
	var mapSettings = {img: gameState.imgUrl, imgData:gameState.imgData, mode: 2, bounds:bounds }
...
usingGoogleMaps = playGameSettings.mapSettings.googleMaps;
	...
	for(var i=1; i< this.fields.length; i++){
		...
		if (type=="travelTime"){
			...
			if (travel == "previous"){
				...
				durMin = Math.round(duration/60  * 10) / 10  //calculate in minutes
			}
			}
			else {
				origin = this.myMap.markers.find(x=>x.metadata.id===parseFloat(travel))
					directionsResult = await calcRoute (origin, marker)
					duration =  directionsResult.routes[0].legs[0].duration.value;
	 			 durMin =  Math.round(duration/60  * 10) / 10     //calculate in minut
			}
...
this.state = data;
try {
handleState(data)  //READ JSON STATE AND SET NEW GAME
}
catch (error) {
