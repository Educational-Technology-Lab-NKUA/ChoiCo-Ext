...
newGame.prototype.pointSettings = function(selectedPointId) {

  for (var i=0; i<this.points.length; i++){
	
      if (this.points[i].id==selectedPointId){
...
newGame.prototype.savePointSettings = function () {
	var myMarkers = myGame.myMap.markers;
	...
	for (var j=0; j< myMarkers.length; j++){
		if(j<myMarkers[j]._leaflet_id == this.currentPoint.id  ){
		 

	  	 oldLayer.removeLayer(myMarkers[j])
   		this.currentPoint.layers = layersList2.value;
  		 newLayer.addLayer(myMarkers[j])
		}
		break;
	}
