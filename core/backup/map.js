...
myGame.changeLayerEvent (layer, myGame.variables);		//log event
}
...
newGame.prototype.addPoints = function(){
	if(!this.myMap.addingPoints){
	$("#addPointIcon").css('backgroundColor', '#75890c')
	$("#map").css('cursor', 'crosshair')
	this.myMap.addingPoints = true;
	}
	else {
		$("#addPointIcon").css('backgroundColor', '')
		this.myMap.addingPoints = false;
		$("#map").css('cursor', 'auto')
	}
}
...
newGame.map.prototype.setDesignMode = function () {
  ...
  this.mapInstance.on('baselayerchange', function(e) {
   this.activeLayer =  getLayerById(e.layer._leaflet_id)
...
var label = "<span id='" + noSpecialChars + "'>" + noSpecialChars + "</span>"
...
newGame.map.prototype.deletePoint = function (leaflet_ID){
		...
		for (var i=0; i<this.markers.length; i++){
				...
				//myGame.modifyPointEvent (myGame.points[i], myGame.myMap, 'delete')		//event logging
				myGame.counters.mapDesignActions ++;
				checkMapActivity (myGame.counters.mapDesignActions);
...
function playClick (e) {					//Click on point on Play Mode
	var imageSkips, tableRows, j, cel, type, pointValue, rowCount;
		myGame.myMap.selectMarker(e.target._leaflet_id)
			...
			myGame.clickedPoints ++;
			 tableRows = myGame.displayTable.rows;
			  if (	tableRows[1].cells.length >1){
