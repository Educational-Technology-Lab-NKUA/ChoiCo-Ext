...
function hideMarker (name){
  ...
  //console.log(name)
      //console.log(this.markers[i].label)
			if(!usingGoogleMaps){
    if ((allMarkers[i].label._content === name)) { 		//chech _icon and not isVisible because older versions don't have is Visible
			allMarkers[i].isVisible = false
			if (allMarkers[i]._icon != null){ //the point is on the current layer
